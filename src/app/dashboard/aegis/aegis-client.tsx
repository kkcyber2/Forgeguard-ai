...
const SHIELD_STORAGE_KEY = "aegis:shield-active";

export default function AegisClientPage() {
  ...
  React.useEffect(() => {
    try {
      setShieldActive(localStorage.getItem(SHIELD_STORAGE_KEY) === "1");
    } catch {
      /* private browsing */
    }
  }, []);
  ...
  function toggleShield() {
    ...
    try {
      localStorage.setItem(SHIELD_STORAGE_KEY, next ? "1" : "0");
    } catch {
      /* private browsing */
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6 pb-12">
      ...
      <motion.div
        ...
        className="rounded-sm border border-white/[0.06] bg-obsidian-800/40 p-4"
      >
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-foreground-muted">
              Engage local shield mode for immediate WAF posture signaling.
            </p>
          </div>
          <button
            type="button"
            role="switch"
            aria-checked={shieldActive}
            onClick={toggleShield}
            className={cn(
              "relative h-7 w-12 rounded-full border transition-colors",
              shieldActive
                ? "border-acid/50 bg-acid/20"
                : "border-white/[0.12] bg-obsidian-700/60",
            )}
          >
            <span
              className={cn(
        ...
        <AnimatePresence mode="wait">
          {shieldActive ? (
            <motion.div
              ...
              className="mt-4 flex items-center gap-2 rounded-xs border border-acid/30 bg-acid/[0.08] px-3 py-2"
            >
              <motion.span
                animate={reduce ? undefined : { opacity: [0.6, 1, 0.6] }}
                transition={{ repeat: Infinity, duration: 2.2, ease: "easeInOut" }}
                className="h-2 w-2 rounded-full bg-acid"
              />
              <span className="font-mono text-[11px] uppercase tracking-[0.12em] text-acid">
                Shield Active
              </span>
            </motion.div>
          ) : (
            <motion.p
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="mt-4 font-mono text-[11px] uppercase tracking-[0.12em] text-foreground-subtle"
            >
              Shield Standby
            </motion.p>
          )}
